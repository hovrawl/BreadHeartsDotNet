...
namespace OpenKh.Egs
{
    public static class EgsTools
    {
        ...
        public static void SetResourcesPath(string path)
        {
	        ...
	        var customNamesPath = Path.Combine(ResourcesPath, "custom_filenames.txt");
	        var kh2NamesPath = Path.Combine(ResourcesPath, "kh2idx.txt");
	        var dddNamesPath = Path.Combine(ResourcesPath, "ddd.txt");
	        var mareNamesPath = Path.Combine(ResourcesPath, "mare.txt");
	        var settingNamesPath = Path.Combine(ResourcesPath, "settingmenu.txt");
	        var theaterNamesPath = Path.Combine(ResourcesPath, "theater.txt");
	        var recomNamesPath = Path.Combine(ResourcesPath, "recom.txt");
	        var bbsNamesPath = Path.Combine(ResourcesPath, "bbs.txt");
	        var kh1pcNamesPath = Path.Combine(ResourcesPath, "kh1pc.txt");
	        var launcher28NamesPath = Path.Combine(ResourcesPath, "launcher28.txt");
	        
	        CustomNames = File.Exists(customNamesPath) ? File.ReadAllLines(customNamesPath).ToList() : new List<string>();
	        KH2Names = File.Exists(kh2NamesPath) ? File.ReadAllLines(kh2NamesPath).ToList() : new List<string>();
	        DddNames = File.Exists(dddNamesPath) ? File.ReadAllLines(dddNamesPath).ToList(): new List<string>();
	        MareNames = File.Exists(mareNamesPath) ? File.ReadAllLines(mareNamesPath).ToList(): new List<string>();
	        SettingMenuNames = File.Exists(settingNamesPath) ? File.ReadAllLines(settingNamesPath).ToList(): new List<string>();
	        TheaterNames = File.Exists(theaterNamesPath) ? File.ReadAllLines(theaterNamesPath).ToList(): new List<string>();
	        RecomNames = File.Exists(recomNamesPath) ? File.ReadAllLines(recomNamesPath).ToList() : new List<string>();
	        BbsNames = File.Exists(bbsNamesPath) ? File.ReadAllLines(bbsNamesPath).ToList() : new List<string>();
	        Kh1AdditionalNames = File.Exists(kh1pcNamesPath) ? File.ReadAllLines(kh1pcNamesPath).ToList() : new List<string>();
	        Launcher28Names = File.Exists(launcher28NamesPath) ? File.ReadAllLines(launcher28NamesPath).ToList() : new List<string>();
	        
	        if (KH2Names != null)
	        {
		        KH2Names = KH2Names?
			        .Concat(KH2Names.Where(x => x.Contains("anm/")).SelectMany(x => new List<string>()
			        {
			        ...
			        .Concat(Kh2.Constants.Languages.SelectMany(lang =>
				        Kh2.Constants.WorldIds.SelectMany(world =>
					        Enumerable.Range(0, 64).Select(index =>
						        Path.Combine("ard", lang).Replace('\\', '/') + $"/{world}{index:D02}.ard"))))
			        .Concat(Kh2.Constants.Languages.SelectMany(lang =>
				        Kh2.Constants.WorldIds.SelectMany(world =>
					        Enumerable.Range(0, 64).Select(index =>
						        Path.Combine("map", lang).Replace('\\', '/') + $"/{world}{index:D02}.map"))))
			        .Concat(Kh2.Constants.Languages.SelectMany(lang =>
				        Kh2.Constants.WorldIds.SelectMany(world =>
					        Enumerable.Range(0, 64).Select(index =>
						        Path.Combine("map", lang).Replace('\\', '/') + $"/{world}{index:D02}.bar"))))
			        .Concat(KH2Names.Where(x => x.StartsWith("bgm/"))
				        .Select(x => x.Replace(".bgm", ".win32.scd")))
			        .Concat(KH2Names.Where(x => x.StartsWith("se/"))
				        .Select(x => x.Replace(".seb", ".win32.scd")))
			        .Concat(KH2Names.Where(x => x.StartsWith("vagstream/"))
				        .Select(x => x.Replace(".vas", ".win32.scd")))
			        .Concat(KH2Names.Where(x => x.StartsWith("gumibattle/se/"))
				        .Select(x => x.Replace(".seb", ".win32.scd")))
			        .Concat(KH2Names.Where(x => x.StartsWith("voice/")).Select(x => x
				        .Replace(".vag", ".win32.scd")
				        .Replace(".vsb", ".win32.scd")));
		        
		        KH2Names = KH2Names?.Concat(new []
		        {
			        "item-011.imd",
			        "KH2.IDX",
			        "ICON/ICON0.PNG",
	        ...
	        var namesToAdd = KH2Names
		        .Concat(DddNames)
		        .Concat(BbsNames)
		        .Concat(RecomNames)
		        .Concat(MareNames)
		        .Concat(SettingMenuNames)
		        .Concat(TheaterNames)
		        .Concat(Kh1AdditionalNames)
		        .Concat(Launcher28Names)
		        .Concat(CustomNames)
		        .Concat(new string[] { "dummy.txt" })
		        .Distinct();
	        foreach (var keyValuePair in namesToAdd.ToDictionary(x => Helpers.ToString(Extensions.GetHashData(Encoding.UTF8.GetBytes(x))), x => x))
	        {
        ...
        public static IEnumerable<string> KH2Names;
        public static List<string> DddNames;
        public static List<string> MareNames;
        public static List<string> SettingMenuNames;
        public static List<string> TheaterNames;
        public static List<string> RecomNames;
        public static List<string> BbsNames;
        public static List<string> Kh1AdditionalNames;
        public static List<string> Launcher28Names;
        ...
        public static void Patch(string pkgFile, string inputFolder, string outputFolder, PatchBackgroundWorker bgw1 = null)
        {
            ...
            {
	            bgw1?.ReportProgress(0, bgw1.PKG + ": " + (hedHeaders.IndexOf(hedHeader)+1) + "/" + hedHeaders.Count);
